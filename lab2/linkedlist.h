...
class LinkedList{
  ...
  LinkedList(){//no parameters. copy constructor has parameters(2 in this case, head and size)
    head=nullptr; 
    size = 0;
  }

/*   linkedlist(int s){ // copy constructor
    head=nullptr; 
   size = s;
  }
 */
...
//all the methods
    ...
    void insertAtPos(string word, int pos){
        if(pos==0){
          ...
          else if(pos >= getSize()|| pos < 0){//prevents seg fault
             return;
          }
/*     if(pos==0){
            insertAtHead(word);
...
int getSize(){
    return size; //dont really need it if you replace all getSize() to size
} 
